    ...
    ReentrancyGuard
{
    ...
    function lock(LockParams calldata params) external payable override nonReentrant returns (uint256) {
        ...
        require(COUNTRY_LIST.countryIsValid(params.countryCode), "COUNTRY"); // countryCode
        FeeStruct memory fee;

        /* 
        根据调用者的身份（msg.sender），决定如何处理收费：
        如果 msg.sender 是 MIGRATE_IN，则从 params.r[0] 中解码出 collectFee。
        否则，根据参数中的 r 解析出费用结构体，或者根据 feeName 获取费用结构体。
        如果存在 flatFee，则扣除该费用。
         */
